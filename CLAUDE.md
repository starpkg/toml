...
1. **No host panics from script input.** Both `unmarshal` and `marshal` wrap the BurntSushi call in a deferred `recover()` that converts a panic into an error. But a stack overflow from deep nesting is a Go *fatal error*, NOT a panic — `recover()` cannot catch it — so a crash-inducing amount of nesting must be rejected *before* the recursive codec runs: `decode` calls `countBracketOpens` on the text (rejecting above `maxParseBrackets`) and `encode` calls `checkEncodeDepth` on the value (against `max_depth`), before `gotoml.Decode` / `dataconv.Unmarshal`. Don't remove either pre-check or the recovers; any new codec call needs the same before-and-after guarding.
2. **Bounded, host-only caps (capwalk).** The decode **crash guard** is `countBracketOpens` vs the fixed `maxParseBrackets` (10000): parser recursion depth can never exceed the number of `[`/`{` openers in the text, so this is a *sound* upper bound that needs **no string lexing** — a deliberate choice, because any precise string-aware scan that diverged from BurntSushi's own lexer would UNDER-count and be a crash bypass (a hand-rolled lexer version was replaced for exactly this reason). Counting openers inside strings/comments only over-counts (fails safe). The user's real limits are enforced *after* a safe parse: `toStarlark` enforces `max_depth`/`max_nodes` on the parsed tree, and `decode` rejects input over `max_input_bytes` up front. `encode` has **two** crash guards, because two recursions run — both against `effectiveEncodeDepth(max_depth)`, clamped into `(0, maxEncodeDepth]` (a fixed absolute ceiling) so the guard is ALWAYS on and its own walk can't overflow even if the host sets `max_depth` to 0 or something enormous: (1) `checkEncodeDepth` walks the **Starlark** value to bound `dataconv.Unmarshal` — via `encodeChildren` (Dict — **both keys and values**, since a nested tuple KEY is recursively hashed/stringified — plus List/Tuple/Set/Struct/Module); it checks the visited-cycle set BEFORE the depth test so a cycle falls through to `dataconv`'s "cyclic reference" error. Host-wrapped `convert.Go*` values are LEAVES to `dataconv`, so they are not walked here. (2) `checkGoDepth` reflect-walks the resulting **Go** value (maps/slices/arrays/exported struct fields, dereferencing ptr/iface) to bound gotoml's encoder — this is what catches a deep value that arrived via a host-wrapped Go leaf. An undercount in EITHER is an encode-crash bypass, so err toward walking; keep both in sync with `dataconv.Unmarshal` and gotoml's encoder respectively. Scope note: for a **pure-Starlark** input (the module's actual use) `Unmarshal`'s result is only maps/slices/scalars, which `checkGoDepth` fully bounds. Exotic Go shapes that only a HOST can smuggle in by wrapping a pathological Go value — cyclic pointer chains (deref is hop-bounded so it can't hang), unexported anonymous embeddings, `toml.Primitive` payloads — are an out-of-scope residual (they'd crash/hang gotoml itself); a host must not expose such values to untrusted scripts. `max_depth`/`max_nodes`/`max_input_bytes` are **host-only** (`SetHostOnly(true)`): `base` emits no `set_<name>` builtin and snapshots the env at construction, so a script cannot widen a cap. Keep the caps host-only, keep the crash guard lexing-free, and keep `checkEncodeDepth` in sync with `dataconv.Unmarshal`.
